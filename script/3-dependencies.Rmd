...
Why the difference? Functions in packages look for variables in a different way to functions in scripts (i.e. functions in the global environment). There is a subtle difference in the scoping rules
...
This same rule is always applied, regardless of whether a function is in a package or script. So what's the difference?
...
Then `document()`, then look at `NAMESPACE`
...
The stats package is special because it's a base package that's always available. That makes life easier. In most cases, however, you will import from other CRAN packages. If you do:
...
What it is telling you is that your `NAMESPACE` depends on a package that is not listed in the `DESCRIPTION`. To fix this: `use_package("ggplot2")`.
...
* Optional, low-benefit, high-cost 
...
```{r}
# http://github.com/r-lib/itdepends/issues/new
...
```{r}
View(itdepends::dep_weight(c("data.table", "dplyr")))
```
...
```{r}
dates <- c(
  "2016-04-20", "1977-08-08", "not a date", "2016",
  "76-03-02", "2012-06-30", "2015-01-21 19:58", NA
)
...
rematch2::re_match(text = dates, pattern = isodate)
```
