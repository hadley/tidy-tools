...
Finish that? Write some documentation and some basic tests.
...
**Your turn** What should `mean()` of an integer return? What should the mean of a date return? How would you write the invariants?

Let's define the basics of the generic.
...
mean2.numeric <- function(x, na.rm = FALSE) {
  if (na.rm) {
    x <- x[!is.na(x)]
  }
...
mean2.POSIXct <- function(x, na.rm = FALSE) {
  new_datetime(mean2(vec_data(x)))
}
```

**Your turn**: Implement `mean2.Date`
